...
func ProductManagerRoutes(incomingRoutes *gin.Engine) {
	...
	authorized.POST("/api/products/add", productController.AddProduct())
	// Edit product from database
	authorized.PUT("/products/edit/:id", productController.EditProduct())
	authorized.PUT("/api/products/edit/:id", productController.EditProduct())
	// Delete product from databse
	authorized.DELETE("/products/delete/:id", productController.DeleteProduct())
	authorized.DELETE("/api/products/delete/:id", productController.DeleteProduct())
	// Get all product from database
	authorized.GET("/products/get", productController.GetAllProducts())
	authorized.GET("/api/products/get", productController.GetAllProducts())
	// search product	authorized.GET("/products/search", productController.GetProductByName())
	authorized.GET("/api/products/search", productController.GetProductByName())

	authorized.GET("/best-selling", productController.GetBestSellingProducts())
	authorized.GET("/api/best-selling", productController.GetBestSellingProducts())
	// get product image
